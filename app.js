(function(context, doc) {
  ...
  const navigation = linkBtnId => {
    ...
    const handleNavLinkClick = (linkObj, event) => {
      ...
      const regionObj = getRegionObj(event.target);
      const expanded = linkObj.getAttribute('aria-expanded') || false;
      const isExpanded = expanded === 'true';
      ...
      if (regionObj) {
        // if ('scrollIntoView' in regionObj) {
        // event.preventDefault();
        // Scroll to the section and set focus to first focusable element
        // regionObj.scrollIntoView({ behavior: 'smooth' });
        // }
        const firstFocusableObj = regionObj.querySelectorAll(
          'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
        )[0];

        if (firstFocusableObj) {
          firstFocusableObj.focus();
        }
      }
    };
